    ...
    Server()
{
    ...
    QString settingsFile = QCoreApplication::applicationDirPath() + "/test.ini";
    QSettings settings(settingsFile, QSettings::IniFormat);

    start(settings.value("CONNECTION/publisher").toString(),
          settings.value("CONNECTION/snapshot").toString(),
          settings.value("CONNECTION/collector").toString());

    int size = settings.beginReadArray("CONTRADE");
    for (int i = 0; i < size; ++i) {
        settings.setArrayIndex(i);
        Contrada c(i,
                   settings.value("NOME").toString(),
                   settings.value("NOMEESTESO").toString().replace("_", " "),
                   settings.value("CARATTERISTICHE").toList());

        m_contrade.append(c);
    }
    settings.endArray();

    size = settings.beginReadArray("CAVALLI");
    for (int i = 0; i < size; ++i) {
        settings.setArrayIndex(i);
        Cavallo c(i,
                   settings.value("NOME").toString(),
    ...
    size = settings.beginReadArray("FANTINI");
    for (int i = 0; i < size; ++i) {
        settings.setArrayIndex(i);
        Fantino f(i,
                  settings.value("NOME").toString(),
  ...
  //nProva = 0;
    ...
    estrazione(Inizio); // inizializza le prime 10 contrade che corrono

    makeDefaultState();
    // Avvia 17 client artificiali
    for(int i=0; i<1; ++i)
        m_aiclients.append(new TDPalioAiClient(m_random));
    //QObject::connect(this, SIGNAL(disconnection(QByteArray)), this, SLOT(handleDisconnection));
    //QObject::connect(this, SIGNAL(newConnection(QByteArray)), this, SLOT(handleNewConnection));
}

void TDPalioServer::makeDefaultState()
{
    // Giocatori
    Players p;
    for (int i=0; i<m_giocatori.size(); i++)
        p.add_giocatori()->CopyFrom(m_giocatori.at(i).toPlayer());
    m_hash[GIOCATORI] = fromProtobuf(p);

    // Contrade
    PContradas c;
    for (int i=0; i<m_contrade.size(); i++) {
        c.add_contrade()->CopyFrom(m_contrade.at(i).toPContrada());
    }
    m_hash[CONTRADE] = fromProtobuf(c);

    // Cavalli
    Horses h;
    for (int i=0; i<m_cavalli.size(); i++)
        h.add_cavalli()->CopyFrom(m_cavalli.at(i).toHorse());
    m_hash[CAVALLI] = fromProtobuf(h);

    // Fantini
    Jockeys j;
    for (int i=0; i<m_fantini.size(); i++)
        j.add_fantini()->CopyFrom(m_fantini.at(i).toJockey());
    m_hash[FANTINI] = fromProtobuf(j);
}

//void TDPalioServer::sendHeartbeat()
//{
//    heartbeat();
//}

void TDPalioServer::messageReceived(const QByteArray& topic, const QByteArray& payload, const QByteArray& uuid)
{
    qDebug() << "new message coming to server";
    if (topic == REGISTER_PLAYER) {
        Player c;
        c.ParseFromArray(payload.data(), payload.size());
        quint8 scelta = scegliContrada(c.contrada());
        Giocatore g;
        g.fromPlayer(c);
        g.setContrada(scelta);
        m_giocatori.append(g);
        Players p;
        for (int i=0; i<m_giocatori.size(); i++)
            p.add_giocatori()->CopyFrom(m_giocatori.at(i).toPlayer());

        qDebug() << "send player choice";
        publish(PLAYERS, fromProtobuf(p), m_uuid.toByteArray());
    }
}

void TDPalioServer::estrazione(TipoEstrazione n)
{
    int con[17];
    QList<unsigned int> sorte;

    switch(n) {
    case Inizio:
        OrdinaI(17, con, m_random);
    ...
    case Ordinario:
        ...
        break;
    case Straordinario:
        OrdinaI(17, settembre, m_random);
        break;
    }
}

int TDPalioServer::scegliContrada(const int& contradaRichiesta)
{
    QList<quint8> prese;
    bool estrai = false;
    for (int i=0; i<m_giocatori.size(); i++) {
        if (contradaRichiesta != -1 && m_giocatori[i].contrada() == contradaRichiesta)
            estrai = true;

        prese.append(m_giocatori[i].contrada());
    }

    if (!estrai && contradaRichiesta != -1)
        return contradaRichiesta;

    QList<quint8> libere;
    for (int i=0; i<17; i++) {
        bool presa = false;
        for (int j=0; j<prese.size(); j++) {
            if (j == i)
                presa = true;
        }
